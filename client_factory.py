...
class ClientFactory:
    ...
    def deploy_nginx(self, subdomain: str, site_path: Path) -> bool:
        """Create an Nginx server block and reload the service.

        Config is written to ``/etc/nginx/sites-available/<subdomain>``
        and symlinked into ``/etc/nginx/sites-enabled/``.

        The server block:
        - Listens on port 80
        - Server name: ``<subdomain>.lavaldigital.ca``
        - Root: ``<site_path>/templates`` (falls back to ``<site_path>``)
        - Index: ``home.html``
        - Includes a security headers snippet and a catch-all location.

        Args:
            subdomain: Client subdomain.
            site_path: Path to the cloned site.

        Returns:
            ``True`` if Nginx was reloaded successfully, ``False`` otherwise.
        """
        root = site_path / "templates"
        if not root.exists():
            root = site_path
    ...
    listen 80;
    listen [::]:80;
    server_name {subdomain}.lavaldigital.ca;
    root {root.resolve()};
    ...
    location /static/ {{
        alias {site_path.resolve()}/static/;
        expires 7d;
...
"""
        available = self.nginx_available / subdomain
        enabled = self.nginx_enabled / subdomain

        try:
            available.write_text(server_block, encoding="utf-8")
            logger.info("Wrote Nginx config: %s", available)

            # Symlink (remove dangling link first if it exists)
            if enabled.exists() or enabled.is_symlink():
                enabled.unlink()
            enabled.symlink_to(available)
            logger.info("Symlinked %s → %s", enabled, available)
            ...
            logger.info("Nginx reloaded successfully for %s", subdomain)
            return True

        except OSError as e:
            logger.error("Failed to deploy Nginx config for %s: %s", subdomain, e)
            return False
        except subprocess.CalledProcessError as e:
            logger.error("systemctl reload nginx failed: %s", e.stderr)
            return False
...
"""
    ...
    def deploy(self, config: dict) -> dict:
        ...
        with self._lock:
            ...
            # ---- Station 6: Nginx ------------------------------------
            try:
                nginx_ok = self.deploy_nginx(subdomain, site_path)
                if not nginx_ok:
